...
class StockMoveLine(models.Model):
    ...
    uom_per_unit = fields.Float(string="")
    total = fields.Float(string="")
...
class StockMoveLine(models.Model):
    ...
    def create(self, vals):
        ...
        if res.sale_line_id:
            res.uom_per_unit = res.sale_line_id.uom_per_unit
            res.total = res.sale_line_id.total
        elif res.purchase_line_id:
            res.uom_per_unit = res.purchase_line_id.uom_per_unit
            res.total = res.purchase_line_id.total
        return res    
    ...
    def calculate_product_qty_return(self):
        if self.product_id and self.product_id.attribute_line_ids:
            for record in self.product_id.attribute_line_ids:
                attr = record.attribute_id
                display_typee = attr.display_type if attr else False
    ...
    def calculate_uom_and_total_return(self):
        if self.product_id and self.product_id.attribute_line_ids:
                for record in self.product_id.attribute_line_ids:
                    attr = record.attribute_id
                    display_typee = attr.display_type if attr else False
